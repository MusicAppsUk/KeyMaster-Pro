...
import './voiceTest.js?v=rc2-206';  // visible Voice Self-Test at #voice-test (no console needed)
import './pwaUpdate.js?v=rc2-206';  // installable-PWA "Update available" flow
...
const VIEW_REGISTRY = {
  ...
  foundations: {
    slot: 'foundations',
    src: './foundations.js?v=rc2-206',
    load: () => import('./foundations.js?v=rc2-206'),
  },
  scales: {
    slot: 'scales',
...
class KeyMasterApp {
  ...
  /**
   * rc2-206: reliable jump to a specific Course lesson (e.g. Course Map -> Key Level 1).
  ...
  _openCourseMap() {
    ...
    import('./foundations.js?v=rc2-206').then((F) => {
    ...
    if (isAudioSupported()) {
      ...
      this.piano = {
        ...
        noteOn: (m, v, t) => {
          ...
          // rc2-206: BLUNT, UNCONDITIONAL boundary counter -- proves whether KL1 demo
          ...
          try {
            if (typeof window !== 'undefined') {
              window.__kmWrapperHits = (window.__kmWrapperHits || 0) + 1;
              const __NMh = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];
              window.__kmLastNote = __NMh[((m % 12) + 12) % 12] + (Math.floor(m / 12) - 1);
              window.__kmLastNoteSrc = window.__kmNoteSrc || '?';
            }
          } catch (_) { /* a counter must NEVER break audio */ }
          // rc2-206: KL1 demo SINGLE-TRIGGER guard at the FINAL note-output boundary.
          ...
          try { if (coursePiano.isReady() && coursePiano.noteOn(m, v, t)) { __eng = 'salamander'; try { if (typeof window !== 'undefined') window.__kmSamplerHits = (window.__kmSamplerHits || 0) + 1; } catch (_) { /* no-op */ } try { window.__kmTraceNote && window.__kmTraceNote(m, __eng, coursePiano); } catch (_) { /* no-op */ } return; } } catch (_) { /* fall through */ }
          try { if (typeof window !== 'undefined') window.__kmFallbackHits = (window.__kmFallbackHits || 0) + 1; } catch (_) { /* no-op */ }
